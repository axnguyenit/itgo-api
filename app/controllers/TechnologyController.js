...
const TechnologyController = {
  ...
  async index(req, res) {
    ...
    if (page) {
      ...
      try {
        ...
        return res.json({ results: technologies, pagination });
      } catch (error) {
    ...
    try {
      ...
      return res.json({ results: technologies });
    } catch (error) {
      console.error(error.message);
      return res.status(500).json({ errors: [{ msg: 'Internal server error' }] });
    }
  ...
  async show(req, res) {
    ...
    try {
      ...
      if (!technology)
        return res.status(400).json({ errors: [{ msg: 'Technology not found' }] });
  ...
  async store(req, res) {
    ...
    try {
      ...
      return res.json({ msg: 'Technology was created successfully' });
    } catch (error) {
      console.error(error);
      return res.status(500).json({ errors: [{ msg: 'Internal server error' }] });
    }
