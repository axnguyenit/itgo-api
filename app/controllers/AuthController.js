...
const AuthController = {
    ...
    try {
      ...
      await transporter.sendMail(mailOptions, function (error, info) {
        if (error) {
          ...
          return res.json({ msg: `We have sent a verify email link to ${email}` });
        }
      });
  ...
  async verifyEmail(req, res) {
    ...
    try {
      ...
      if (user.emailVerified) return res.json({ msg: 'Permission allowed' });

      const secret = `${process.env.ACCESS_TOKEN_SECRET}${user.emailVerified}`;
      await JWT.verify(token, secret);
      await User.findByIdAndUpdate(id, { emailVerified: true });

      return res.json({ msg: 'Permission allowed' });
    } catch (error) {
      console.error(error.message);
      return res.status(500).json({ errors: [{ msg: 'Internal server error' }] });
    }
    ...
    try {
      ...
      await transporter.sendMail(mailOptions, function (error, info) {
        if (error) {
          ...
          return res.json({ msg: `We have sent a reset password link to ${email}` });
        }
      });
    } catch (error) {
      console.error(error.message);
